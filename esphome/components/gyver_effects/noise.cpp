...
#define P(x) progmem_read_byte(p + x)
...
ESPHOME_ALWAYS_INLINE static inline int16_t grad16(uint8_t hash, int16_t x, int16_t y, int16_t z) {
  hash = hash & 15;
...
ESPHOME_ALWAYS_INLINE static inline int16_t grad16(uint8_t hash, int16_t x, int16_t y) {
  hash = hash & 7;
  int16_t u, v;
  if (hash < 4) {
    u = x;
...
ESPHOME_ALWAYS_INLINE static inline int16_t grad16(uint8_t hash, int16_t x) {
  hash = hash & 15;
  int16_t u, v;
  if (hash > 8) {
    u = x;
...
ESPHOME_ALWAYS_INLINE static inline int8_t selectBasedOnHashBit(uint8_t hash, uint8_t bitnumber, int8_t a, int8_t b) {
  return (hash & (1 << bitnumber)) ? a : b;
}

ESPHOME_ALWAYS_INLINE static inline int8_t grad8(uint8_t hash, int8_t x, int8_t y, int8_t z) {
  hash &= 0xF;
  int8_t u, v;
  // u = (hash&8)?y:x;
...
ESPHOME_ALWAYS_INLINE static inline int8_t grad8(uint8_t hash, int8_t x, int8_t y) {
...
ESPHOME_ALWAYS_INLINE static inline int8_t grad8(uint8_t hash, int8_t x) {
  // since the tests below can be done bit-wise on the bottom
...
ESPHOME_ALWAYS_INLINE static inline int8_t lerp7by8(int8_t a, int8_t b, uint8_t frac) {
  // int8_t delta = b - a;
...
void fill_raw_2dnoise8(uint8_t *pData, int width, int height, uint8_t octaves, q44 freq44, uint8_t amplitude, int skip,
                       uint16_t x, int16_t scalex, uint16_t y, int16_t scaley, uint16_t time) {
  if (octaves > 1) {
    fill_raw_2dnoise8(pData, width, height, octaves - 1, freq44, amplitude, skip + 1, x * freq44, freq44 * scalex,
                      y * freq44, freq44 * scaley, time);
  } else {
...
void fill_raw_2dnoise8(uint8_t *pData, int width, int height, uint8_t octaves, uint16_t x, int scalex, uint16_t y,
                       int scaley, uint16_t time) {
  fill_raw_2dnoise8(pData, width, height, octaves, q44(2, 0), 128, 1, x, scalex, y, scaley, time);
}

void fill_raw_2dnoise16(uint16_t *pData, int width, int height, uint8_t octaves, q88 freq88, uint16_t amplitude,
                        int skip, uint32_t x, int32_t scalex, uint32_t y, int32_t scaley, uint32_t time) {
  if (octaves > 1) {
    fill_raw_2dnoise16(pData, width, height, octaves - 1, freq88, amplitude, skip, x * freq88, scalex * freq88,
                       y * freq88, scaley * freq88, time);
  } else {
    // amplitude is always 255 on the lowest level
    amplitude = 65535;
  }
  ...
  scalex *= skip;
  scaley *= skip;
  uint16_t invamp = 65535 - amplitude;
...
void fill_raw_2dnoise16into8(uint8_t *pData, int width, int height, uint8_t octaves, q44 freq44, uint8_t amplitude,
                             int skip, uint32_t x, int32_t scalex, uint32_t y, int32_t scaley, uint32_t time) {
  if (octaves > 1) {
    fill_raw_2dnoise16into8(pData, width, height, octaves - 1, freq44, amplitude, skip + 1, x * freq44, scalex * freq44,
                            y * freq44, scaley * freq44, time);
  } else {
    // amplitude is always 255 on the lowest level
    amplitude = 255;
  }
...
void fill_raw_2dnoise16into8(uint8_t *pData, int width, int height, uint8_t octaves, uint32_t x, int scalex, uint32_t y,
                             int scaley, uint32_t time) {
  fill_raw_2dnoise16into8(pData, width, height, octaves, q44(2, 0), 171, 1, x, scalex, y, scaley, time);
}

void fill_noise8(GRGB *leds, int num_leds, uint8_t octaves, uint16_t x, int scale, uint8_t hue_octaves, uint16_t hue_x,
  ...
  for (int j = 0; j < num_leds; j += 255) {
    ...
    for (int i = 0; i < LedsPer; ++i) {
      leds[i + j] = GHSV(H[i], 255, V[i]);
    }
  }
}

void fill_noise16(GRGB *leds, int num_leds, uint8_t octaves, uint16_t x, int scale, uint8_t hue_octaves, uint16_t hue_x,
                  int hue_scale, uint16_t time, uint8_t hue_shift) {
  if (num_leds <= 0)
    return;
  ...
  for (int j = 0; j < num_leds; j += 255) {
    ...
    for (int i = 0; i < LedsPer; ++i) {
      leds[i + j] = GHSV(H[i] + hue_shift, 255, V[i]);
    }
  }
}

void fill_2dnoise8(GRGB *leds, int width, int height, bool serpentine, uint8_t octaves, uint16_t x, int xscale,
                   uint16_t y, int yscale, uint16_t time, uint8_t hue_octaves, uint16_t hue_x, int hue_xscale,
                   uint16_t hue_y, uint16_t hue_yscale, uint16_t hue_time, bool blend) {
  ...
  for (int i = 0; i < height; ++i) {
    ...
    for (int j = 0; j < width; ++j) {
      GRGB led = GHSV(H[h1 - i][w1 - j], 255, V[i][j]).to_rgb();
...
void fill_2dnoise16(GRGB *leds, int width, int height, bool serpentine, uint8_t octaves, uint32_t x, int xscale,
                    uint32_t y, int yscale, uint32_t time, uint8_t hue_octaves, uint16_t hue_x, int hue_xscale,
                    uint16_t hue_y, uint16_t hue_yscale, uint16_t hue_time, bool blend, uint16_t hue_shift) {
  uint8_t V[height][width];
  uint8_t H[height][width];
  ...
  for (int i = 0; i < height; ++i) {
    int wb = i * width;
    for (int j = 0; j < width; ++j) {
      GRGB led = GHSV(hue_shift + (H[h1 - i][w1 - j]), 196, V[i][j]).to_rgb();
